...
class SuperResolver:
    ...
    def __call__(self, frame):
        lr = cv2.resize(frame, (self.in_w, self.in_h)).astype(np.uint8)[None]
        self.interp.set_tensor(self.in_det['index'], lr)
        self.interp.invoke()
        sr = self.interp.get_tensor(self.out_det['index'])[0]
        return cv2.resize(sr, (frame.shape[1], frame.shape[0]))